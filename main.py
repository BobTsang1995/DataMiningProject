...
Test_data = pd.read_csv('used_car_testA_20200313.csv', sep=' ', encoding="utf-8")
...
tree_data[0:int(train_num)].to_csv('train_tree.csv', index=0, sep=' ')
tree_data[train_num:train_num+50000].to_csv('text_tree.csv', index=0, sep=' ')
...
feature = ['model', 'brand', 'bodyType', 'fuelType', 'kilometer', 'notRepairedDamage', 'power',
           'regDate_month', 'creatDate_year', 'creatDate_month'
...
print(nn_data.shape)
train_num = df.shape[0]-50000
nn_data[0:int(train_num)].to_csv('train_nn.csv', index=0, sep=' ')
nn_data[train_num:train_num+50000].to_csv('test_nn.csv', index=0, sep=' ')
...
sub.to_csv('lgb_test.csv', index=False)
...
sub['price'] = oof_lgb
sub.to_csv('lgb_train.csv', index=False)
...
sub['SaleID'] = TestA_data.SaleID
sub['price'] = predictions
sub.to_csv('cab_test.csv', index=False)
...
sub['SaleID'] = Train_data.SaleID
sub['price'] = oof_cb
sub.to_csv('cab_train.csv', index=False)
...
Train_NN_data = pd.read_csv('train_nn.csv', sep=' ')
Test_NN_data = pd.read_csv('test_nn.csv', sep=' ')
...
sub['price'] = predictions
sub.to_csv('nn_test.csv', index=False)
...
sub['price'] = oof_nn
sub.to_csv('nn_train.csv', index=False)


# 导入树模型lgb预测数据，进行二层stacking输出
predictions_lgb = np.array(pd.read_csv('lgb_test.csv')['price'])
oof_lgb = np.array(pd.read_csv('lgb_train.csv')['price'])

# 导入树模型cab预测数据，进行二层stacking输出
predictions_cb = np.array(pd.read_csv('cab_test.csv')['price'])
...
predictions_nn = np.array(pd.read_csv('nn_test.csv')['price'])
oof_nn = np.array(pd.read_csv('nn_train.csv')['price'])
